...
# Sugar Glider failure-path test:
...
DLQ_GROUP="sugar-glider-dlq-chaos"
DLQ_CONSUMER="sugar-glider-dlq-chaos-consumer"
WAL_TIMEOUT_SEC=120
DLQ_TIMEOUT_SEC=40
HTTP_TIMEOUT_SEC=8
...
usage() {
    cat <<'EOF'
Usage: sugar_glider_failure_test.sh [options]

Options:
  --file <compose-file>          Compose file (default: docker-compose.rtg-sugar-glider.local.yml)
  --url <sugar-glider-url>            Sugar Glider URL (default: http://localhost:8081)
...
sugar_glider_config() {
    curl -fsS --max-time "${HTTP_TIMEOUT_SEC}" "${SUGAR_GLIDER_URL}/v1/config"
}
...
main() {
    ...
    log "Step 1/3: Checking Sugar Glider readiness"
    local ready
    ready="$(curl -fsS --max-time "${HTTP_TIMEOUT_SEC}" "${SUGAR_GLIDER_URL}/readyz" 2>/dev/null || true)"
    echo "${ready}" | jq -e '.ready == true' >/dev/null 2>&1 || fail "Sugar Glider is not ready at ${SUGAR_GLIDER_URL}"
    ...
    wal_before="$(sugar_glider_config | jq -r '.metrics.wal_replayed // 0')"
    ...
    publish_body="$(jq -cn \
        ...
        --arg sender "sugar-glider-failure-test" \
    ...
    publish_json="$(mktemp)"
    publish_status="$(curl -sS -o "${publish_json}" -w '%{http_code}' --max-time "${HTTP_TIMEOUT_SEC}" -X POST "${SUGAR_GLIDER_URL}/v1/publish" -H 'Content-Type: application/json' -d "${publish_body}" || true)"
    ...
    while [ "${elapsed}" -lt "${WAL_TIMEOUT_SEC}" ]; do
        wal_depth="$(sugar_glider_config | jq -r '.wal_depth // -1')"
    ...
    wal_after="$(sugar_glider_config | jq -r '.metrics.wal_replayed // 0')"
    ...
    dlq_entry_id="$(redis_cmd XADD "${STREAM}" '*' event_type "chaos.dlq.test" sender "sugar-glider-failure-test" priority "normal" payload "${dlq_payload}" timestamp "$(date -u +%FT%TZ)" | tail -n1 | tr -d '\r')"
