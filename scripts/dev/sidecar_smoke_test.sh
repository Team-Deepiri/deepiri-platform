...
usage() {
    cat <<'EOF'
Usage: sidecar_smoke_test.sh [options]

Options:
  --url <sidecar-url>         Sidecar base URL (default: http://localhost:8081)
  --stream <name>             Redis stream name (default: platform-events)
  --group <name>              Consumer group name (default: sidecar-smoke)
...
main() {
    ...
    log "Checking sidecar readiness: ${SIDECAR_URL}/readyz"
    ready_payload="$(curl -fsS --max-time 5 "${SIDECAR_URL}/readyz")" || {
        echo "Sidecar readiness endpoint is unavailable." >&2
        exit 1
    }
    if ! echo "${ready_payload}" | jq -e '.ready == true' >/dev/null 2>&1; then
        echo "Sidecar is not ready: ${ready_payload}" >&2
        exit 1
    fi

    smoke_id="smoke-$(date +%s)-${RANDOM}"
    payload_json="$(jq -cn --arg smoke_id "${smoke_id}" --arg source "sidecar_smoke_test" --arg ts "$(date -u +%FT%TZ)" '{smoke_id: $smoke_id, source: $source, timestamp: $ts}')"
    publish_body="$(jq -cn \
        --arg stream "${STREAM}" \
        --arg event_type "smoke.test" \
        --arg sender "sidecar-smoke-test" \
    ...
    publish_resp="$(curl -fsS --max-time 8 -X POST "${SIDECAR_URL}/v1/publish" -H 'Content-Type: application/json' -d "${publish_body}")"
    ...
    for attempt in $(seq 1 "${ATTEMPTS}"); do
        read_resp="$(curl -fsS --max-time 10 -X POST "${SIDECAR_URL}/v1/read" -H 'Content-Type: application/json' -d "${read_body}")"
        ...
        if [ -n "${match_id}" ]; then
            ...
            ack_resp="$(curl -fsS --max-time 8 -X POST "${SIDECAR_URL}/v1/ack" -H 'Content-Type: application/json' -d "${ack_body}")"
            acked="$(echo "${ack_resp}" | jq -r '.acked // 0')"
            if [ "${acked}" -ge 1 ]; then
                log "PASS: sidecar smoke test succeeded (publish/read/ack)"
                exit 0
            fi
