...
ensure_submodule_on_main() {
    ...
    # Check current branch — skip switching if already on a feature branch (not detached)
    local current_branch=$(git symbolic-ref --short HEAD 2>/dev/null || echo "")
    if [ -n "$current_branch" ] && [ "$current_branch" != "$branch" ]; then
        echo "    ℹ️  On feature branch '$current_branch', fetching $branch without switching..."
    ...
    if ! git symbolic-ref -q HEAD > /dev/null; then
        ...
        # Discard any changes to tracked files (hooks are regenerated by setup-hooks.sh)
...
# Helper: update submodule to remote only if not on a feature branch
update_submodule_remote() {
    local submodule_path="$1"
    if [ ! -d "$submodule_path" ]; then return; fi
    local current_branch
    current_branch=$(cd "$submodule_path" && git symbolic-ref --short HEAD 2>/dev/null || echo "")
    if [ -n "$current_branch" ] && [ "$current_branch" != "main" ] && [ "$current_branch" != "master" ]; then
        echo "    ℹ️  Skipping remote update for '$submodule_path' (on feature branch '$current_branch')"
        return
    fi
    git submodule update --remote -f "$submodule_path"
}

# Update to latest and ensure on main branch
echo "🔄 Updating submodules to latest and ensuring they're on main branch..."
update_submodule_remote "deepiri-core-api"
ensure_submodule_on_main "deepiri-core-api"
update_submodule_remote "platform-services/backend/deepiri-api-gateway"
ensure_submodule_on_main "platform-services/backend/deepiri-api-gateway"
update_submodule_remote "platform-services/backend/deepiri-auth-service"
ensure_submodule_on_main "platform-services/backend/deepiri-auth-service"
update_submodule_remote "platform-services/backend/deepiri-external-bridge-service"
ensure_submodule_on_main "platform-services/backend/deepiri-external-bridge-service"
update_submodule_remote "platform-services/backend/deepiri-language-intelligence-service"
ensure_submodule_on_main "platform-services/backend/deepiri-language-intelligence-service"
update_submodule_remote "deepiri-web-frontend"
ensure_submodule_on_main "deepiri-web-frontend"
update_submodule_remote "platform-services/shared/deepiri-prismpipe"
