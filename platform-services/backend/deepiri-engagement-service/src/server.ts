...
mongoose.connect(MONGO_URI)
  .then(() => logger.info('Engagement Service: Connected to MongoDB'))
  .catch((err: Error) => logger.error('Engagement Service: MongoDB connection error', err));
...
const errorHandler: ErrorRequestHandler = (err, req, res, next) => {
  logger.error('Engagement Service error:', err);
