/**
 * Event Consumer for Incentive Engine.
 *
...
const CONSUMER_NAME = process.env.REDIS_CONSUMER_NAME || `incentive-${process.pid}`;

export async function startEventConsumption(): Promise<void> {
  if (isConsuming) {
    secureLog('warn', '[Incentive] Event consumption already started');
    return;
  }
  ...
  try {
    ...
    secureLog('info', '[Incentive] Connected to Redis Streams');

    consumePlatformEvents().catch((err) => {
      secureLog('error', '[Incentive] Platform events consumption error:', err);
    });

    isConsuming = true;
    secureLog('info', '[Incentive] Event consumption started');
  } catch (error) {
    secureLog('error', '[Incentive] Failed to start event consumption:', error);
    throw error;
  }
}
      ...
      try {
        switch (event.event) {
          case 'task-completed':
            ...
            secureLog('info', `[Incentive] Awarded 10 momentum to ${userId} for task-completed`);
            break;
          ...
          case 'task-created':
            ...
            secureLog('info', `[Incentive] Awarded 2 momentum to ${userId} for task-created`);
            break;
          ...
          case 'user-registered':
            ...
            secureLog('info', `[Incentive] Awarded 5 welcome momentum to ${userId}`);
            break;
        }
      } catch (err) {
        // Log but never rethrow — a single-user error must not kill the consumer loop
        secureLog('error', `[Incentive] Error processing ${event.event} for user ${userId}:`, err);
      }
    },
    {
      consumerGroup: 'incentive-engine',
...
export async function stopEventConsumption(): Promise<void> {
  if (streamingClient) {
    ...
    isConsuming = false;
    secureLog('info', '[Incentive] Event consumption stopped');
  }
}
