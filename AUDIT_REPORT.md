...
### Frontend team delivery (PR — `Team-Deepiri/deepiri-web-frontend` #192, branch `yves_kwawu/feat/cloud-alignment`)
The fixes above landed on the personal copy (`main`@`ffe197b`). Because the CD pipeline builds the portal from the `Team-Deepiri` frontend submodule (`main@0c46037`, a restructured tree: `AuthContext`/`SocketContext`, `src/api`, no `platformClient`/zustand), they were **ported** to a fresh branch off org `main`:
...
> Deploy: after #192 merges to `main`, bump the `deepiri-web-frontend` submodule pointer here and set `VITE_REALTIME_GATEWAY_URL` when the realtime gateway is provisioned.
